		...
		<section id="contents">
			...
			<div id="weather_div">
				<div class="front">
					<c:choose>
						<c:when test="${empty list }">
							...
							<p style="color: gray; margin-top: 10px;">서비스 가능지역에 따라 정보 제공이 안될 수도 있습니다. </p>
						</c:when>
						<c:otherwise>
							<h3>${ citys}${counties } ${ town} 의 날씨입니다.</h3>
					<h4>${dateInput }기준으로 조회했습니다</h4>
					<p style="color: gray; margin-top: 10px;"">※조회 시간은 매시간 30분이며, ${front_time }으로
						조회했습니다</p>
						</c:otherwise>
					</c:choose>
				...
				<div class="weather-categories-container">
					...
					<div class="weather-category LGT">
						<h4>낙뢰</h4>
						<ul>
							<c:forEach var="k" items="${list }">
								<c:if test="${k.category eq 'LGT' }">
									<li>${k.fcstTime}kA</li>
								</c:if>
							</c:forEach>
						</ul>
