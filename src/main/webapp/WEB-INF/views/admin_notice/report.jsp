...
//삭제게시물 검색
function searchDeletedReport() {
    $.ajax({
        url: '/adrep_deleted.do',
        type: 'GET',
        success: function(response) {
            $("#bal").html(response);
        },
...
$(document).ready(function() {
    $("#del_btn").click(function() { // 클릭 이벤트 핸들러 설정
        ...
        $("input[name='chk']:checked").each(function() { // 체크된 체크박스 찾기
        	selectedRep.push($(this).val()); // 선택된 NOTICE_NUM을 배열에 추가
        });

        // Ajax 요청을 보냅니다.
        $.ajax({
            type: "POST", // 또는 GET
            url: "/adrep_deleted1.do", // 다른 컨트롤러의 URL을 여기에 입력
            data: { selectedRep: selectedRep }, // 선택된 NOTICE_NUM 값을 전송
            success: function(response) {
...
<body>
	...
	<div class="search_wrap">
		<div
			...
			<h1 style="text-align: center; font-size: 18px;">전체 신고 게시글 : ${totalReport}개</h1>
			</p>
			<br>
			<p>
			<h1 style="text-align: center; font-size: 18px;">삭제한 신고 게시글 : ${deletedReport}개</h1>
			</p>
		</div>
				...
				<div>
					...
					<div>
						<span
							...
							onclick="searchDeletedReport()"></span> 
						...
						<dt>
							<div>
								<span style="float: right; margin-top: 130px; margin-right: -495px;">
								 ...
								 <input type="hidden" value="신고" name="mg_type">
