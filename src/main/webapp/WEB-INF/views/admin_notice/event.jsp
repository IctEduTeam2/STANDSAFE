...
//테이블 삭제 버튼
$(document).ready(function() {
    $("#deleve_btn").click(function() { // 클릭 이벤트 핸들러 설정
        var selectedEvent = [];
        $("input[name='chk']:checked").each(function() { // 체크된 체크박스 찾기
        	selectedEvent.push($(this).val()); // 선택된 NOTICE_NUM을 배열에 추가
        });

        // Ajax 요청을 보냅니다.
        $.ajax({
            type: "POST", // 또는 GET
            url: "/adevent_deleted1.do", // 다른 컨트롤러의 URL을 여기에 입력
            data: { selectedEvent: selectedEvent }, // 선택된 NOTICE_NUM 값을 전송
            success: function(response) {
...
//홈페이지 등록
$(document).ready(function() {
    $("#homeeve_btn").click(function() { // 클릭 이벤트 핸들러 설정
        var selectedEvent = [];
        $("input[name='chk']:checked").each(function() { // 체크된 체크박스 찾기
        	selectedEvent.push($(this).val()); // 선택된 NOTICE_NUM을 배열에 추가
        });
        ...
        $.ajax({
            type: "POST", // 또는 GET
            url: "/update_adeventstatus.do", // 다른 컨트롤러의 URL을 여기에 입력
            data: { selectedEvent: selectedEvent }, // 선택된 NOTICE_NUM 값을 전송
            success: function(response) {
                // Ajax 요청이 성공했을 때 실행할 코드
                // 서버에서 반환한 응답(response)을 처리합니다.
...
<body>
	...
	<div class="search_wrap">
		<div
			...
			<h1 style="text-align: center; font-size: 18px;">전체 이벤트 게시글 : ${totalEvent}개</h1>
			</p>
			<br>
			<h1 style="text-align: center; font-size: 18px;">등록한 이벤트 게시글 : ${registeredEvent}개</h1>
			</p>
			<br>
			<p>
			<h1 style="text-align: center; font-size: 18px;">삭제한 이벤트 게시글 : ${deletedEvent}개</h1>
			</p>
		</div>
				...
				<div>
					...
					<div>
						<span
							...
							onclick="searchDeletedEvent()"></span> 
						...
						<dt>
							<div>
								<span style="float: right; margin-top: 130px; margin-right: -495px;">
								 ...
								 <input type="hidden" value="EVENT" name="mg_type">
	...
	<div>
		<span style="float: right; margin-top: 25px; margin-right: 50px;">
    		  <button type="button" alt="삭제" value="삭제"
                ...
                id="deleve_btn">삭제</button>
		</span>
		<span style="float: right; margin-top: 25px; margin-right: 50px;">
			<button type="button" alt="글쓰기" value="글쓰기"
				style="width: 150px; height: 50px; font-size: 16px; border-radius: 10px; background-color: #505BBD; color: white; border: none;"
				onclick="location.href='/ad_eventform.do'">글쓰기</button>
		</span> 
		<span style="float: right; margin-top: 25px; margin-right: 50px;">
			<button type="button" alt="홈페이지 등록" value="홈페이지 등록"
                style=" width: 150px; height: 50px; font-size: 16px; border-radius: 10px; background-color: #505BBD; color: white; border: none;"
                id="homeeve_btn">홈페이지 등록</button>
		</span>
	</div>
