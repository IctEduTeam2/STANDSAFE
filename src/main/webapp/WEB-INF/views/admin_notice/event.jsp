...
#btn4{background:#1b5ac2}
.menu{margin-bottom: 50px}
</style>
<script type="text/javascript">
	function allevent_go(f) {
		f.action="/ad_eveall_go.do";
		f.submit();
	}
	function writeevent_go(f) {
		f.action="/ad_evewrite_go.do";
		f.submit();
	}
</script>
</head>
...
<jsp:include page="header.jsp"></jsp:include>
		...
		<h1 style="text-align: center; font-size: 18px;">전체 이벤트 게시글 : 125개</h1></p>
		<br>
		<h1 style="text-align: center; font-size: 18px;">등록한 이벤트 게시글 : 125개</h1></p>
		<br>
		<p><h1 style="text-align: center; font-size: 18px;">삭제한 게시글 : 5개</h1></p>
		</div>		
		<!-- 검색 영역 -->
			...
			<div
				...
				<div>
					<dl style="margin-top: 40px;">
						...
						<dd>
							<p>
								<span> <span style="font-family: '맑은 고딕'; font-size: 16px; float: left; margin-left: 50px;">검색어 &nbsp;</span>
								<select id="searchKey" name="searchKey" title="검색항목선택" class="select_option" style="margin-left: 55px; width: 300px; height: 50px; font-size: 20px;">
										...
										<option value="title2">회원 아이디</option>
										<option value="title3">내용</option>
								</select>	 
								</span>
	...
	<thead>
		...
		<form method="post" enctype="multipart/form-data" id="form">
			<span style="float: right; margin-top: 25px; margin-right: 180px;">
				<button type="button" alt="이벤트" value="이벤트"
				style="width: 150px; height: 50px; font-size: 16px; border-radius: 10px; background-color: #505BBD; color: white; border: none;" onclick="javascript:allevent_go(this.form)">이벤트❐</button>
			</span> <span style="float: right; margin-top: 25px; margin-right: 50px;">
				<button type="button" alt="글쓰기" value="글쓰기"
				style="width: 150px; height: 50px; font-size: 16px; border-radius: 10px; background-color: #505BBD; color: white; border: none;"onclick="javascript:writeevent_go(this.form)">글쓰기</button>
			</span> <span style="float: right; margin-top: 25px; margin-right: 50px;">
				<button type="button" alt="홈페이지 등록" value="홈페이지 등록"
				style="width: 150px; height: 50px; font-size: 16px; border-radius: 10px; background-color: #505BBD; color: white; border: none;" onclick="addevent()">홈페이지 등록</button>
			</span>
		</form>
		</div>
		<!-- 수평선 추가1 -->
