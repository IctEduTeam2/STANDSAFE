...
//하단테이블 삭제 버튼
$(document).ready(function() {
    $("#delqa_btn").click(function() { // 클릭 이벤트 핸들러 설정
        var selectedQa = [];
        $("input[name='chk']:checked").each(function() { // 체크된 체크박스 찾기
        	selectedQa.push($(this).val()); // 선택된 NOTICE_NUM을 배열에 추가
        });

        // Ajax 요청을 보냅니다.
        $.ajax({
            type: "POST", // 또는 GET
            url: "/adqa_deleted1.do", // 다른 컨트롤러의 URL을 여기에 입력
            data: { selectedQa: selectedQa }, // 선택된 NOTICE_NUM 값을 전송
            success: function(response) {
...
<body>
	...
	<div class="search_wrap">
		<div
			...
			<h1 style="text-align: center; font-size: 18px;">전체 Q&A 게시글 : ${totalQa}개</h1>
			</p>
			<br>
			<p>
			<h1 style="text-align: center; font-size: 18px;">삭제한 Q&A 게시글 : ${deletedQa}개</h1>
			</p>
		</div>
		
		<!-- 검색 영역 -->
		<form id="searchForm" action="/adqa_search.do" method="post" onsubmit="setStartField()">
						...
						<dt>
							<div>
								<span style="float: right; margin-top: 130px; margin-right: -495px;">
								 ...
								 <input type="hidden" value="QA" name="mg_type">
